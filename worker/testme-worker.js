 ...
 *   POST /auth/register      { user, pass, email }     → הרשמה, מחזיר token
 *   POST /auth/login         { user, pass }            → התחברות, מחזיר token
 *   POST /auth/set-email     { token, email }          → חיבור אימייל לחשבון קיים
 *   POST /auth/forgot-username  { email }              → שולח שם משתמש למייל
 *   POST /auth/forgot-password  { email }              → שולח קוד איפוס למייל
 *   POST /auth/reset-password   { email, code, newPass } → קובע סיסמה חדשה
 *   POST /data/save           { token, data }          → שמירת תוצאות אישיות
 *   POST /data/load           { token }                → טעינת תוצאות אישיות
 * סיסמאות: hash חד-כיווני (PBKDF2 100k) — לעולם לא טקסט.
 */
